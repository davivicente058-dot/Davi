...
--====================================================--
-- FPS DEFINITIVO - PARTE 5 (MASTER PERFORMANCE PLUS)
-- Frame Pacing • Anti Stutter • Smooth Render • Ultra Fluidez
-- COLAR ABAIXO DA PARTE 4
--====================================================--
...
local MASTER_PLUS = {
    ...
    -- Frame pacing (quanto maior, mais suave)
    TargetFrameTime = 1 / 60, -- base 60hz (não trava FPS)

    -- Anti-spike
    SpikeThreshold = 0.035, -- tempo máximo por frame antes de corrigir

    -- Garbage Collector
    GCStepInterval = 3, -- segundos
    GCAgressivo = true,

    -- Render
    RenderThrottle = true,
    RenderSkipFactor = 0, -- 0 = não pula frames
    ...
    -- Debug interno (não mostra nada visual)
    Debug = false
}
...
local lastGC = tick()
local smoothDelta = MASTER_PLUS.TargetFrameTime
local spikeBuffer = {}

-- ================= FUNÇÕES =================

local function smoothFrameDelta(raw)
    smoothDelta = smoothDelta + (raw - smoothDelta) * 0.1
    return smoothDelta
end

local function antiSpike(delta)
    if delta > MASTER_PLUS.SpikeThreshold then
        table.insert(spikeBuffer, delta)
        if #spikeBuffer > 5 then
            table.remove(spikeBuffer, 1)
        end
        return MASTER_PLUS.TargetFrameTime
    end
    return delta
end

local function runGarbageCollector()
    if tick() - lastGC >= MASTER_PLUS.GCStepInterval then
        lastGC = tick()
        if MASTER_PLUS.GCAgressivo then
            collectgarbage("collect")
        else
...
local function optimizePhysics()
    if not MASTER_PLUS.PhysicsReduce then return end
    settings().Physics.PhysicsEnvironmentalThrottle = Enum.EnviromentalPhysicsThrottle.Disabled
    settings().Physics.AllowSleep = true
end

local function prioritizeInput()
    if not MASTER_PLUS.InputPriority then return end
...
RunService.RenderStepped:Connect(function(delta)
    if not MASTER_PLUS.Enabled then return end

    -- Frame pacing
    local now = tick()
    local rawDelta = now - lastRender
    lastRender = now

    local adjusted = smoothFrameDelta(rawDelta)
    adjusted = antiSpike(adjusted)

    -- Evita processamento excessivo
    if MASTER_PLUS.RenderThrottle and MASTER_PLUS.RenderSkipFactor > 0 then
        if math.random(1, MASTER_PLUS.RenderSkipFactor + 1) ~= 1 then
            return
        end
    end
    ...
    -- Micro-delay controlado (simula VSync suave)
    local waitTime = MASTER_PLUS.TargetFrameTime - adjusted
    if waitTime > 0 and waitTime < 0.01 then
        task.wait(waitTime)
    end
end)

-- ================= FUNÇÕES EXPONÍVEIS =================
-- Permite integração com UI das partes anteriores

_G.FPS_MASTER_PLUS = _G.FPS_MASTER_PLUS or {}

function _G.FPS_MASTER_PLUS.Toggle(state)
    MASTER_PLUS.Enabled = state
end

function _G.FPS_MASTER_PLUS.SetTargetFPS(fps)
    if fps and fps >= 30 and fps <= 240 then
        MASTER_PLUS.TargetFrameTime = 1 / fps
    end
end

function _G.FPS_MASTER_PLUS.UltraLowLatency(state)
    MASTER_PLUS.RenderThrottle = not state
    MASTER_PLUS.RenderSkipFactor = state and 0 or 1
end

-- Estado padrão
_G.FPS_MASTER_PLUS.Toggle(true)

print("[FPS DEFINITIVO] PARTE 5 (MASTER PLUS) carregada com sucesso.")
