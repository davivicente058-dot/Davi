-- FPS Buster V2 (Robusto para Mobile / Delta)
-- UI visível, notificações, FPS real, ping, otimizações, restauração, minimização
-- Substitua o RAW atual por este arquivo e execute no Delta.
...
local function notify(title, text, dur)
    ...
    -- também printa para console
    print(("[FPS Buster] %s: %s"):format(tostring(title), tostring(text)))
end
...
local function applyOptimizations()
    ...
    otimizado = true
    notify("FPS Buster", "Otimização aplicada", 2)
end
...
local function pauseHeavyTasks(state)
    ...
    -- Placeholder: se você tiver conexões pesadas, desconecte aqui
end
...
-- =========================
-- UI (robusta e visível no mobile)
-- =========================
local function createUI()
    ...
    local playerGui = safe(function() return LocalPlayer:WaitForChild("PlayerGui", 8) end) or LocalPlayer:FindFirstChild("PlayerGui")
    if not playerGui then
        notify("FPS Buster", "PlayerGui não disponível (timeout)", 4)
        -- tenta continuar sem UI
        return
    end

    -- remove GUI antiga
    local existing = playerGui:FindFirstChild("FPSBusterUI")
    if existing then existing:Destroy() end

    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "FPSBusterUI"
